...
当你以另一个 hostname 提供 DeepTutor 服务时（云部署、局域网访问、反向代理），用户浏览器需要一个自己能访问的 backend URL。

修复：打开 **设置 -> Network**，或把 `data/user/settings/system.json` 里的 `next_public_api_base_external` 设成对外可达的 URL：
...
然后重启 DeepTutor。`public_api_base` 仍作为兼容别名接受，保存时会归一化到 `next_public_api_base_external`。
...
```bash
pip install -e ".[partners]"           # 全部内置 channel SDK（源码安装）
pip install -U "deeptutor[partners]"   # PyPI 安装用这条
pip install -e ".[matrix]"             # Matrix channel；加密房间用 ".[matrix-e2e]"（需要 libolm）
```
...
针对每个 gateway 的调试方法见 [**Partners 伙伴与渠道**](/zh-cn/docs/partners/) 和 [**渠道矩阵**](/zh-cn/docs/partners/channels/)。
